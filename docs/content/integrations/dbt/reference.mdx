...
## Generate row count data for dbt models

<Note>
  <strong>
    Emitting row count data for dbt models is currently an experimental
    feature.{" "}
  </strong>{" "}
...
Dagster has the ability to automatically fetch row counts for dbt-generated tables and emit them as [materialization metadata](/concepts/metadata-tags/asset-metadata) to be displayed in the Dagster UI.

Row counts are fetched in parallel to the execution of your dbt models. To enable this feature, call [`fetch_row_counts()`](/\_apidocs/libraries/dagster-dbt#dagster_dbt.core.resources_v2.DbtEventIterator.fetch_row_counts) on the [`DbtEventIterator`](/\_apidocs/libraries/dagster-dbt#dagster_dbt.core.resources_v2.DbtEventIterator) returned by the `stream()` dbt CLI call:
...
## Emit column-level lineage as materialization metadata

<Note>
  <strong>
    Emitting column-level lineage as materialization metadata is currently an
    experimental feature.{" "}
  </strong>{" "}
  To use this feature, you'll need to be on at least <code>dagster>=1.7.0</code>{" "}
  and
  <code>dagster-dbt>=0.23.0</code>.
</Note>

Dagster allows you to emit column-level metadata, like column schema and column dependencies, as [materialization metadata](/concepts/metadata-tags/asset-metadata).
